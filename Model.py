...
class HGDM(nn.Module):
    ...
    def forward(self):
        ...
        main_view = all_heter_embeddings + torch.concat([target_user_embedding,target_item_embedding],dim=0)
        main_user_view = main_view[:self.n_user]
        main_item_view = main_view[self.n_user:]
        return target_user_embedding, target_item_embedding, heter_user_embedding, heter_item_embedding,main_user_view, main_item_view
    ...
    def cal_loss(self, ancs, poss, negs):
        ...
        poss = poss.to(device)
        negs = negs.to(device)
        usrEmbeds, itmEmbeds, h_usrEmbeds, h_itemEmbeds,main_user_view, main_item_view = self.forward()
        u_diff_loss,diff_usrEmbeds= self.diffusion_process.training_losses2(self.usr_denoiser, usrEmbeds, h_usrEmbeds, ancs)  # diff_usrEmbeds、diff_itemEmbeds：去噪后的嵌入表示,u_diff_loss、i_diff_loss：用户和物品的去噪损失
        ...
        contrastive_view = torch.concat([usrEmbeds ,itmEmbeds], dim=0)
        contrastive_user_view = contrastive_view[:usrEmbeds.size(0)]
        contrastive_item_view = contrastive_view[usrEmbeds.size(0):]
        ...
        contrastive_loss = self.contrastive_loss(main_user_view, contrastive_user_view, ancs,
                                                 temperature=args.temperature)
        contrastive_loss =contrastive_loss+self.contrastive_loss(main_item_view, contrastive_item_view, poss,
                                                  temperature=args.temperature)
        contrastive_loss *= args.ssl_reg
        loss = bprLoss + regLoss + diff_loss+contrastive_loss

        return loss,bprLoss,regLoss,diff_loss,contrastive_loss

    def contrastive_loss(self, main_view, contrastive_view, indices, temperature=args.temperature):
        device = main_view.device
        main_view = main_view.to(device)
        contrastive_view = contrastive_view.to(device)
        indices = indices.to(device)
        main_embeddings = main_view[indices]
        contrastive_embeddings = contrastive_view[indices]
        sim_matrix = F.cosine_similarity(main_embeddings.unsqueeze(1),
                                         contrastive_embeddings.unsqueeze(0),
                                         dim=-1)

        sim_matrix = sim_matrix / temperature
        logits = torch.exp(sim_matrix)
        loss = -torch.log(logits.diag() / logits.sum(dim=1))
        return loss.mean()
    ...
    def predict(self):
        ...
        d_ui_embedings = torch.concat([denoised_u,denoised_i],dim=0)
        return usrEmbeds, itmEmbeds,d_ui_embedings
...
class DGLLayer(nn.Module):
    ...
    def forward(self, graph, u_f, v_f):
        with graph.local_scope():
            ...
            # graph.edata['e_f'] = e_f
            graph.update_all(fn.copy_u(u='n_f', out='m'), reduce_func=fn.sum(msg='m', out='n_f'))  # 使用 DGL 的 update_all 方法，将归一化后的节点特征 node_f 作为消息传递给所有邻居节点，并累加到每个节点上
...
class GaussianDiffusion(nn.Module):
    ...
    def training_losses2(self, model, targetEmbeds, x_start, batch):
        ...
        targetEmbeds = targetEmbeds.to(device)
        ...
        weight = torch.where((ts == 0), 1.0, weight)
        diff_loss = weight * mse
        batch=batch.to(weight.device)
        batch = batch.to(diff_loss.device)
        diff_loss=weight[batch]
        return diff_loss,model_output
