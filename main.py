...
class Coach:
    ...
    def trainEpoch(self):
        ...
        epLoss, epRecLoss, eDiffLoss, eContrLoss = 0, 0, 0, 0
        steps = trnLoader.dataset.__len__() // args.batch
        mode = 'all_diff'
        for i, tem in enumerate(trnLoader):
            ancs, poss, negs = tem
            ancs = ancs.to(device).long()
            poss = poss.long().cuda()
            negs = negs.long().cuda()

            self.opt.zero_grad()

            loss, bprLoss, regLoss, diff_loss, constrastive_loss = self.model.cal_loss(ancs, poss,negs)  # 总损失，包括基于贝叶斯个性化排序（BPR）的损失、正则化损失（regularization loss）以及扩散模型的去噪损失（diffusion loss）
            epLoss += loss.item()
            epRecLoss += bprLoss.item()
            eDiffLoss += diff_loss.item()
            eContrLoss += constrastive_loss.item()

            loss.backward()
            self.opt.step()

            log('Step %d/%d: loss = %.3f, diffLoss = %.3f, regLoss = %.3f, contrastiveLoss = %.3f' % (
                i, steps, loss.item(), diff_loss.item(), regLoss.item(), constrastive_loss.item()), save=False,
                oneline=True)
        ...
        ret['ContrLoss'] = eContrLoss / steps
        return ret
    ...
    def testEpoch(self):
        ...
        with torch.no_grad():
            usrEmbeds, itmEmbeds, uiEmbeds = self.model.predict()
        ...
        for usr, trnMask in tstLoader:
            ...
            topLocs_cpu = topLocs.cpu().numpy()  # 将 topLocs 转移到 CPU
