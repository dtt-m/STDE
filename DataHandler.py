...
class DataHandler:
    ...
    def _load_data(self):
        ...
        for i in range(len(self.behaviors_data)):
            self.beh_degree_list.append(torch.tensor(((self.behaviors_data[i] != 0) * 1).sum(axis=-1)).to(device))
    ...
    def load_data(self):
        ...
        self.train_dataloader = dataloader.DataLoader(train_dataset, batch_size=args.batch, shuffle=True, num_workers=4,
                                                      pin_memory=True)
